...
## Step 13: インサイトDB・入力UI

### 目的

投稿ごとのSNSインサイト数値を手動入力・保存できるようにする。

### 13-1. DBマイグレーション

#### `post_insights`（投稿インサイト）
...
| メソッド | パス | 説明 |
|---------|------|------|
| GET | `/api/clients/[clientId]/posts/[postId]/insights` | インサイト取得（なければ null） |
| POST | `/api/clients/[clientId]/posts/[postId]/insights` | インサイト新規作成（upsert） |
| PATCH | `/api/clients/[clientId]/posts/[postId]/insights` | インサイト更新 |
...
- [ ] clientロールはインサイト閲覧のみ（編集不可）

---

## Step 14: インサイトダッシュボード（グラフ）

### 目的

クライアントごとの月次インサイトを可視化し、代理店・クライアント双方が「数値の流れ」をひと目で把握できるようにする。
...
| パス | 説明 | アクセス |
|------|------|---------|
| `/clients/[clientId]/insights` | 月次インサイトダッシュボード | 企業管理者 / スタッフ / クライアント |
...
components/insights/
  ...
  post-insights-table.tsx     -- 投稿別インサイト一覧テーブル
...
- [ ] 月次インサイトダッシュボードページが表示される
- [ ] リーチ推移折れ線グラフが描画される
- [ ] KPIサマリーカード（月間リーチ・平均保存率・平均ホーム率・投稿数）が表示される
- [ ] レーダーチャートで目標値との比較が表示される
- [ ] 投稿別インサイト一覧テーブルが表示される
- [ ] 月選択で過去の月のデータを確認できる
- [ ] インサイト未入力の投稿はハイライト（まだ入力なし）で表示される
- [ ] サイドバーに「インサイト」リンクが追加される
...
入力されたインサイト数値をもとに、OpenAI gpt-4o-mini で投稿単位の改善提案を生成する。
...
**リクエスト**: インサイトデータ（post_insights の内容）+ 投稿情報（タイトル・キャプション・投稿種別）
...
- [ ] インサイト保存後に「提案を生成する」ボタンが表示される
- [ ] 生成ボタンクリックで3件の改善提案が表示される
- [ ] カテゴリ別アイコン・優先度バッジが表示される
- [ ] ローディング状態が適切に表示される
- [ ] インサイトデータが未入力の場合はボタンが無効化される
...
- [ ] 日本語が文字化けしない
- [ ] インサイトデータが0件の月でも「データなし」として出力できる
...
| メソッド | パス | 説明 | ロール制限 |
|---------|------|------|----------|
| GET | `/api/clients/[clientId]/posts/[postId]/insights` | インサイト取得 | 全ロール（閲覧） |
| POST | `/api/clients/[clientId]/posts/[postId]/insights` | インサイト作成/更新 | staff / agency_admin / master |
...
| ステップ | 内容 | 推定工数 |
|---------|------|---------|
| Step 13 | インサイトDB・入力UI | 2日 |
| Step 14 | インサイトダッシュボード（グラフ） | 2日 |
| Step 15 | AI改善提案 | 0.5日 |
...
### パフォーマンス
- インサイトダッシュボードのグラフは `"use client"` コンポーネントで描画
